...
| 70 | `70-docker-limits-critical.yml` | CPU / memory / PID guardrails for critical services: override.local.yml + immediate `docker update` (no restart). | yes |
| 99 | `99-verify.yml` | Read-only health gate: system metrics, Docker/container state, UFW, app dirs, host guardrail files, systemd timers, and restricted TCP listeners. Emits `reports/health/<ts>.json` + `reports/verify-*.md`. | **no** |
...
|---|---|---|
| `./verify.sh` | Wrapper over `99-verify.yml`. Full health and host guardrail check in seconds. | Before / after any mutating action |
...
| [`vps-runtime-map.md`](vps-runtime-map.md) | Channel-supporting VPS runtime map: host objects, versions, listener schemes, Channel M/D guardrails, test diagrams | Cross-project routing/VPS review |
| [`containers.md`](containers.md) | Container inventory with owner, image, ports, and resource limits | Resource planning, new services |
| [`ports.md`](ports.md) | Canonical port map (source of truth for `port-audit`) | Adding a service; resolving conflicts |
| [`firewall.md`](firewall.md) | UFW rules, trusted IPs, public Console exposure, and documented non-UFW bridge-scoped exceptions such as Channel M reverse | Security review |
